...
Type Algorithme::getProgType(std::string prog) const {
	...
	if(id >= 0 && id < programmes.size()) {
		...
		throw "Aucun programme #"+int2str(id)+" n'existe (gPT).";
	}
}
...
void Algorithme::setProgVar(int id, Variable& var) {
	if(id >= 0 && id < programmes.size()) {
		programmes[id].setVariable(var);
	}
	else {
		throw "Aucun programme #"+int2str(id)+" n'existe (sPV).";
	}
}
